    ...
    <div class="w-full">
      ...
      <header class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-slate-500 text-sm">
            إدارة ومتابعة كافة الوحدات السكنية وحالاتها
          </p>
        </div>
      ...
      <div class="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <div class="group relative overflow-hidden rounded-xl border border-white bg-white p-4 shadow-sm">
          ...
          <p class="relative text-xs font-bold text-slate-400">
            إجمالي الوحدات
          </p>
      ...
      <div class="rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
        ...
        <div class="border-b border-slate-100 bg-slate-50/30 p-3">
          <div class="flex flex-col md:flex-row gap-3 justify-between items-center">
            ...
            <div class="flex gap-2">
              ...
              <button
                class="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-medium hover:bg-slate-200 transition-colors"
              >
                تصفية
              </button>
            </div>
        ...
        >
          ...
          <p class="mt-1 text-slate-500 text-sm">
            يمكنك إضافة وحدة جديدة بالضغط على الزر أعلاه
          </p>
        </div>
          ...
          <table class="w-full border-collapse min-w-[900px] text-sm">
            ...
            <thead>
              <tr class="bg-slate-50 border-b border-slate-200">
                ...
                <th class="px-3 py-3 text-center text-xs font-bold text-slate-600">الغرف</th>
              ...
              >
                ...
                <td class="px-3 py-3">
                  ...
                  <div
                    ...
                    class="text-xs text-slate-400 mt-0.5 max-w-[150px] truncate"
                  >
                    {{ unit.description }}
                  </div>
                </td>
                ...
                <td class="px-3 py-3 text-center text-slate-600 text-xs">
                  {{ unit.area || '—' }} م²
                </td>

                <td class="px-3 py-3 text-emerald-600 font-bold text-xs">
        ...
        <div
          ...
          class="border-t border-slate-200 px-4 py-3 flex flex-col sm:flex-row justify-between items-center gap-3 bg-slate-50/30"
        >
...
export default {
  ...
  computed: {
    ...
    filteredUnits() {
      ...
      if (this.searchQuery) {
        ...
        result = result.filter(unit =>
          ...
          // unit.project_name?.toLowerCase().includes(query)
        )
      }

      if (this.statusFilter) {
        result = result.filter(
          unit => unit.status === this.statusFilter
        )
      }
    ...
    totalPages() {
      return Math.ceil(
        this.filteredUnits.length / this.pageSize
      )
    },
    ...
    totalPrice() {
      return this.units.reduce(
        (acc, unit) =>
           acc + (parseFloat(unit.total_price) || 0),
        0
      )
    }
